...
namespace Server
{
	...
	public class LootPackEntry
	{
		...
		{
			if ( item is GlassSword || item is EarringsOfTheVile || item is QuiverOfInfinity || 
			item is JesterHatofChuckles || item is ShaMontorrossbow || item is EarringsOfProtection || 
			item is SamuraiHelm || item is ArcticDeathDealer || item is LuckyNecklace || 
			item is BlazeOfDeath || item is BowOfTheJukaKing || item is HolySword || 
			item is BurglarsBandana || item is CavortingClub || item is DupresShield || 
			item is EnchantedTitanLegBone || item is GwennosHarp || item is EarringBoxSet || 
			item is IolosLute || item is LunaLance || item is SilvanisFeywoodBow || 
			item is NightsKiss || item is NoxRangersHeavyCrossbow || item is TheNightReaper || 
			item is OrcishVisage || item is PolarBearMask || item is BlightGrippedLongbow || 
			item is ShieldOfInvulnerability || item is StaffOfPower || item is ColdForgedBlade || 
			item is VioletCourage || item is HeartOfTheLion || item is LuminousRuneBlade || 
			item is WrathOfTheDryad || item is PixieSwatter || item is OverseerSunderedBlade || 
			item is GlovesOfThePugilist || item is DreadPirateHat || item is PhantomStaff || 
			item is CandelabraOfSouls || item is ColdBlood || item is RuneCarvingKnife || 
			item is ArcaneArms || item is GlovesOfAegis || item is JackalsTunic || 
			item is ArcaneCap || item is GlovesOfFortune || item is LeggingsOfAegis || 
			item is ArcaneGloves || item is GlovesOfInsight || item is LeggingsOfFire || 
			item is ArcaneGorget || item is GlovesOfTheFallenKing || item is LegsOfFortune || 
			item is ArcaneLeggings || item is GlovesOfTheHarrower || item is LegsOfInsight || 
			item is ArcaneTunic || item is GorgetOfAegis || item is LegsOfNobility || 
			item is ArmorOfInsight || item is GorgetOfFortune || item is LegsOfTheFallenKing || 
			item is ArmorOfNobility || item is GorgetOfInsight || item is LegsOfTheHarrower || 
			item is ArmsOfAegis || item is HelmOfAegis || item is MidnightGloves || 
			item is ArmsOfFortune || item is HolyKnightsArmPlates || item is MidnightHelm || 
			item is ArmsOfInsight || item is HolyKnightsGloves || item is MidnightLegs || 
			item is ArmsOfNobility || item is HolyKnightsGorget || item is MidnightTunic || 
			item is ArmsOfTheFallenKing || item is HolyKnightsLegging || item is RingOfHealth || 
			item is ArmsOfTheHarrower || item is HolyKnightsPlateHelm || item is RingOfTheMagician || 
			item is BraceletOfTheElements || item is HuntersArms || item is ShadowDancerArms || 
			item is BraceletOfTheVile || item is HuntersGloves || item is ShadowDancerCap || 
			item is CapOfFortune || item is HuntersGorget || item is ShadowDancerGloves || 
			item is CapOfTheFallenKing || item is HuntersLeggings || item is ShadowDancerGorget || 
			item is CoifOfBane || item is HuntersTunic || item is ShadowDancerTunic || 
			item is CoifOfFire || item is InquisitorsArms || item is TotemArms || 
			item is DivineArms || item is InquisitorsGorget || item is TotemGloves || 
			item is DivineGloves || item is InquisitorsHelm || item is TotemGorget || 
			item is DivineGorget || item is InquisitorsLeggings || item is TotemLeggings || 
			item is DivineLeggings || item is InquisitorsTunic || item is TotemTunic || 
			item is DivineTunic || item is JackalsArms || item is TunicOfAegis || 
			item is EarringsOfHealth || item is JackalsGloves || item is TunicOfBane || 
			item is EarringsOfTheElements || item is JackalsHelm || item is TunicOfTheFallenKing || 
			item is EarringsOfTheMagician || item is JackalsLeggings || item is TunicOfTheHarrower || 
			item is YashimotosHatsuburi || item is WizardsPants || item is Fortifiedarms || 
			item is WarriorsClasp || item is MauloftheBeast || item is FesteringWound || 
			item is WeaponRenamingTool || item is MarbleShield || item is FalseGodsScepter || 
			item is MagiciansMempo || item is EvilMageGloves || 
			item is VampiricDaisho || item is MagiciansIllusion || item is EternalFlame || 
			item is TownGuardsHalberd || item is MagesBand || item is DupresCollar || 
			item is SwiftStrike || item is MadmansHatchet || item is DeathsMask || 
			item is SprintersSandals || item is LuckyEarrings || item is DarkNeck || 
			item is ShimmeringTalisman || item is LongShot || item is DarkLordsPitchfork || 
			item is ShadowBlade || item is LeggingsOfEnlightenment || item is CircletOfTheSorceress || 
			item is RoyalGuardsGorget || item is LeggingsOfDeceit || item is DarkGuardiansChest || 
			item is RoyalGuardsChestplate || item is KamiNarisIndestructableDoubleAxe || item is BookOfKnowledge || 
			item is RoyalArchersBow || item is JinBaoriOfGoodFortune || item is BalancingDeed || 
			item is RobeOfTreason || item is JadeScimitar || item is AuraOfShadows || 
			item is Retort || item is Indecency || item is ArmsOfToxicity || 
			item is RamusNecromanticScalpel || item is HellForgedArms || item is ArcticBeacon || 
			item is PowerSurge || item is GlovesOfRegeneration || item is ArcanicRobe || 
			item is Pestilence || item is GlovesOfCorruption || item is Annihilation || 
			item is NoxNightlight || item is GeishasObi || item is AngeroftheGods || 
			item is NoxBow || item is Fury || item is AngelicEmbrace || 
			item is NordicVikingSword || item is FurCapeOfTheSorceress || item is AbysmalGloves || 
			item is MinersPickaxe || item is FortunateBlades || item is RobeOfTheEquinox || 
			item is VoiceOfTheFallenKing || item is Quell || item is ArcaneShield || 
			item is TunicOfFire || item is OrcChieftainHelm || item is ZyronicClaw || 
			item is ShadowDancerLeggings || item is GladiatorsCollar || item is TitansHammer || 
			item is OrnateCrownOfTheHarrower || item is FangOfRactus || item is TheTaskmaster || 
			item is MidnightBracers || item is CrownOfTalKeesh || item is TheDryadBow || 
			item is LeggingsOfBane || item is Calm || item is TheDragonSlayer || 
			item is JackalsCollar || item is AcidProofRobe || item is TheBeserkersMaul || 
			item is InquisitorsResolution || item is SpiritOfTheTotem || item is StaffOfTheMagi || 
			item is HolyKnightsBreastplate || item is HuntersHeaddress || item is SerpentsFang || 
			item is HelmOfInsight || item is HatOfTheMagi || item is LegacyOfTheDreadLord || 
			item is GauntletsOfNobility || item is DivineCountenance || item is Frostbringer || 
			item is ArmorOfFortune || item is CrimsonCincture || item is BreathOfTheDead || 
			item is TheRobeOfBritanniaAri || item is RingOfTheVile || item is QuiverOfElements || 
			item is SamaritanRobe || item is RingOfTheElements || item is QuiverOfRage || 
			item is RoyalGuardSurvivalKnife || item is OrnamentOfTheMagician || item is RighteousAnger || 
			item is OblivionsNeedle || item is BraceletOfHealth || item is RobeOfTheEclipse || 
			item is LieutenantOfTheBritannianRoyalGuard || item is QuiverOfLightning || 
			item is GuantletsOfAnger || item is QuiverOfIce || item is SoulSeeker || 
			item is EmbroideredOakLeafCloak || item is QuiverOfFire || item is TalonBite || 
			item is DjinnisRing || item is QuiverOfBlight || item is WildfireBow || 
			item is Subdue || item is ElvenQuiver || item is TotemOfVoid || 
			item is ShroudOfDeciet || item is Aegis || item is Windsong || 
			item is EverlastingLoaf || item is EverlastingBottle || item is PolarBearCape || 
			item is PolarBearBoots || item is Excalibur || item is KodiakBearMask || 
			item is Stormbringer || item is AchillesShield || item is AchillesSpear || 
			item is StaffofSnakes || item is AilricsLongbow || item is BeltofHercules || 
			item is LargeBagofHolding || item is MediumBagofHolding || item is SmallBagofHolding || item is BagOfHolding || 
			item is RobinHoodsFeatheredHat || item is RobinHoodsBow || item is HammerofThor || 
			item is AxeoftheMinotaur || item is BowofthePhoenix || item is GandalfsRobe || 
			item is GandalfsHat || item is GandalfsStaff || item is PandorasBox || 
			item is ConansHelm || item is ConansSword || item is SinbadsSword || 
			item is SkullChest || item is ShardThrasher || item is BeggarsRobe || 
			item is VampiresRobe || item is ConansLoinCloth || item is PendantOfTheMagi || 
			item is EssenceOfBattle || item is ResilientBracer || item is GiantBlackjack || 
			item is QuiverOfRage || item is MelisandesCorrodedHatchet || item is BloodwoodSpirit || 
			item is GrayMouserCloak || item is BootsofHermes || item is FleshRipper || 
			item is CandleCold || item is CandleFire || item is CandlePoison || 
			item is CandleEnergy || item is CandleWizard || item is CandleNecromancer || 
			item is GrimReapersRobe || item is GrimReapersMask || item is GrimReapersLantern || 
			item is GrimReapersScythe || item is DragonLegs || item is DragonGloves || 
			item is DragonArms || item is DragonChest || item is DragonHelm || 
			item is MagicBoneLegs || item is MagicBoneGloves || item is MagicBoneArms || 
			item is MagicFurLegs || item is MagicFurArms || item is MagicFurChest || 
			item is TorchOfTrapFinding || item is RodOfResurrection || item is RobeOfTeleportation || 
			item is MaulOfTheTitans || item is HelmOfBrilliance || item is GlovesOfDexterity || 
			item is GemOfSeeing || item is DaggerOfVenom || item is ColoringBook || 
			item is MagicBoneChest || item is MagicBoneHelm || item is MagicBoneSkirt ){ return true; }
		...
		public Item Construct( bool inIslesDread )
		{
			try
			{
				...
				if ( m_Type == typeof( BaseRanged ) )
					...
					item = Loot.RandomArmorOrHat( inIslesDread );
					...
					item = Loot.RandomGem();
				else if ( m_Type == typeof( YashimotosHatsuburi ) )
				{
