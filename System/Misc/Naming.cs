...
namespace Server
{
	public class NameList
	{
		...
		static NameList()
		{
			...
			string filePath = Path.Combine( Core.BaseDirectory, "Data/XML/names.xml" );
...
namespace Server.Gumps
{
    ...
    {
        public CustomTitleGump(Mobile from) : base(25, 25)
        {
            this.Closable=true;
			this.Disposable=true;
			this.Dragable=true;
			...
			AddImage(552, 11, 143);
			AddImage(16, 348, 159);
			AddTextEntry(126, 238, 267, 20, 1511, 1, @"Type here...", 25);
			AddHtml( 175, 37, 349, 180, @"<BODY><BASEFONT Color=#FBFBFB><BIG>" + text + " So if you have an idea for a unique character title, then delete the text below and retype it. A new title can be no longer than 25 characters.</BIG></BASEFONT></BODY>", (bool)false, (bool)false);
			AddButton(90, 238, 4005, 4005, 1, GumpButtonType.Reply, 0);
        }
        ...
        public override void OnResponse( NetState sender, RelayInfo info )
        {
			...
			if ( name.Contains("Legendary ") || name.Contains("Elder ") || name.Contains("Grandmaster ") || name.Contains("Master ") || name.Contains("Adept ") || name.Contains("Expert ") || name.Contains("Journeyman ") || name.Contains("Apprentice ") || name.Contains("Novice ") || name.Contains("Neophyte ") )
            {
