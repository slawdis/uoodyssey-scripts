...
namespace Server.Misc
{
    class MyServerSettings
    {
		...
		private static bool S_AllowSaveFunction = false;
		...
		private static bool S_CreaturesDetectHidden = true;
		...
		private static bool S_ClientVersion = true;
        ...
        public static void Initialize()
        {
			if ( System.IO.File.Exists( "Info/settings.xml" ) )
			{
				...
				if ( root != null )
				{
					...
					{
						if ( setting == 1 ){ S_AllowSaveFunction = bool.Parse(node.InnerText); }
						else if ( setting == 2 ){ S_ServerSaveMinutes = XmlConvert.ToDouble(node.InnerText); }
						else if ( setting == 3 ){ S_SaveOnCharacterLogout = bool.Parse(node.InnerText); }
						else if ( setting == 4 ){ S_PersistentBlackjack = bool.Parse(node.InnerText); }
						else if ( setting == 5 ){ S_FloorTrapTrigger = Int32.Parse(node.InnerText); }
						else if ( setting == 6 ){ S_GetUnidentifiedChance = Int32.Parse(node.InnerText); }
						else if ( setting == 7 ){ S_NoMacroing = bool.Parse(node.InnerText); }
						else if ( setting == 8 ){ S_StatGain = XmlConvert.ToDouble(node.InnerText); }
						else if ( setting == 9 ){ S_StatGainDelay = XmlConvert.ToDouble(node.InnerText); }
						else if ( setting == 10 ){ S_PetStatGainDelay = XmlConvert.ToDouble(node.InnerText); }
						else if ( setting == 11 ){ S_GetTimeBetweenQuests = Int32.Parse(node.InnerText); }
						else if ( setting == 12 ){ S_GetTimeBetweenArtifactQuests = Int32.Parse(node.InnerText); }
						else if ( setting == 13 ){ S_GetGoldCutRate = Int32.Parse(node.InnerText); }
						else if ( setting == 14 ){ S_AllowMacroResources = bool.Parse(node.InnerText); }
						else if ( setting == 15 ){ S_CreaturesDetectHidden = bool.Parse(node.InnerText); }
						else if ( setting == 16 ){ S_GuardsSentenceDeath = bool.Parse(node.InnerText); }
						else if ( setting == 17 ){ S_GuardsPatrolOutside = bool.Parse(node.InnerText); }
						else if ( setting == 18 ){ S_GuardsSprint = bool.Parse(node.InnerText); }
						else if ( setting == 19 ){ S_NoMountsInCertainRegions = bool.Parse(node.InnerText); }
						else if ( setting == 20 ){ S_AllowAlienChoice = bool.Parse(node.InnerText); }
						else if ( setting == 21 ){ S_AnnounceTrapSaves = bool.Parse(node.InnerText); }
						else if ( setting == 22 ){ S_IdentifyItemsOnlyInPack = bool.Parse(node.InnerText); }
						else if ( setting == 23 ){ S_DamageToPets = XmlConvert.ToDouble(node.InnerText); }
						else if ( setting == 24 ){ S_CriticalToPets = Int32.Parse(node.InnerText); }
						else if ( setting == 25 ){ S_SpellDamageIncreaseVsMonsters = Int32.Parse(node.InnerText); }
						else if ( setting == 26 ){ S_SpellDamageIncreaseVsPlayers = Int32.Parse(node.InnerText); }
						else if ( setting == 27 ){ S_RunRoutinesAtStartup = bool.Parse(node.InnerText); }
						else if ( setting == 28 ){ S_QuestRewardModifier = Int32.Parse(node.InnerText); }
						else if ( setting == 29 ){ S_PlayerLevelMod = XmlConvert.ToDouble(node.InnerText); }
						else if ( setting == 30 ){ S_WyrmBody = Int32.Parse(node.InnerText); }
						else if ( setting == 31 ){ S_Quest = bool.Parse(node.InnerText); }
						else if ( setting == 32 ){ S_FastFriends = bool.Parse(node.InnerText); }
						else if ( setting == 33 ){ S_FriendsAvoidHeels = bool.Parse(node.InnerText); }
						else if ( setting == 34 ){ S_FriendsGuardFriends = bool.Parse(node.InnerText); }
						else if ( setting == 35 ){ S_BoatDecay = XmlConvert.ToDouble(node.InnerText); }
						else if ( setting == 36 ){ S_HomeDecay = XmlConvert.ToDouble(node.InnerText); }
						else if ( setting == 37 ){ S_HousesDecay = bool.Parse(node.InnerText); }
						else if ( setting == 38 ){ S_HousesPerAccount = Int32.Parse(node.InnerText); }
						else if ( setting == 39 ){ S_EnableDungeonSoundEffects = bool.Parse(node.InnerText); }
						else if ( setting == 40 ){ S_SellChance = Int32.Parse(node.InnerText); }
						else if ( setting == 41 ){ S_SellCommonChance = Int32.Parse(node.InnerText); }
						else if ( setting == 42 ){ S_SellRareChance = Int32.Parse(node.InnerText); }
						else if ( setting == 43 ){ S_SellVeryRareChance = Int32.Parse(node.InnerText); }
						else if ( setting == 44 ){ S_BuyChance = Int32.Parse(node.InnerText); }
						else if ( setting == 45 ){ S_BuyCommonChance = Int32.Parse(node.InnerText); }
						else if ( setting == 46 ){ S_BuyRareChance = Int32.Parse(node.InnerText); }
						else if ( setting == 47 ){ S_ShinyArmor = bool.Parse(node.InnerText); }
						else if ( setting == 48 ){ S_NewLeather = bool.Parse(node.InnerText); }
						else if ( setting == 49 ){ S_Leopards = bool.Parse(node.InnerText); }
						else if ( setting == 50 ){ S_Elephants = bool.Parse(node.InnerText); }
						else if ( setting == 51 ){ S_Zebras = bool.Parse(node.InnerText); }
						else if ( setting == 52 ){ S_Fox = bool.Parse(node.InnerText); }
						else if ( setting == 53 ){ S_Basements = bool.Parse(node.InnerText); }
						else if ( setting == 54 ){ S_ClientVersion = bool.Parse(node.InnerText); }
						else if ( setting == 55 ){ S_NoMountBuilding = bool.Parse(node.InnerText); }
						else if ( setting == 56 ){ S_NoMountsInHouses = bool.Parse(node.InnerText); }
						else if ( setting == 57 ){ S_AllowCustomTitles = bool.Parse(node.InnerText); }
		...
		public static bool AllowSaveFunction()
		{
			...
			return S_AllowSaveFunction;
		}

		public static double ServerSaveMinutes() // HOW MANY MINUTES BETWEEN AUTOMATIC SERVER SAVES
		{
			if ( S_ServerSaveMinutes > 240 ){ S_ServerSaveMinutes = 240.0; }
			else if ( S_ServerSaveMinutes < 10 ){ S_ServerSaveMinutes = 10.0; }
		...
		public static int FloorTrapTrigger()
		{
			...
			if ( S_FloorTrapTrigger < 5 ){ S_FloorTrapTrigger = 5; }
		...
		public static int GetUnidentifiedChance()
		{
			...
			if ( S_GetUnidentifiedChance < 10 ){ S_GetUnidentifiedChance = 10; }
		...
		public static TimeSpan PetStatGainDelay()
		{
			...
			if ( S_PetStatGainDelay > 60 ){ S_PetStatGainDelay = 60.0; } else if ( S_PetStatGainDelay < 1 ){ S_PetStatGainDelay = 1.0; }
		...
		{
			if ( S_GetTimeBetweenQuests > 240 ){ S_GetTimeBetweenQuests = 240; } else if ( S_GetTimeBetweenQuests < 1 ){ S_GetTimeBetweenQuests = 0; }
		...
		{
			if ( S_GetTimeBetweenArtifactQuests > 20160 ){ S_GetTimeBetweenArtifactQuests = 20160; } else if ( S_GetTimeBetweenArtifactQuests < 1 ){ S_GetTimeBetweenArtifactQuests = 0; }
		...
		public static int GetGoldCutRate() // DEFAULT IS 25% OF WHAT GOLD NORMALLY DROPS
		{
			...
			if ( S_GetGoldCutRate < 5 ){ S_GetGoldCutRate = 5; } else if ( S_GetGoldCutRate > 100 ){ S_GetGoldCutRate = 100; }
		...
		public static bool CreaturesDetectHidden()
		{
			...
			return S_CreaturesDetectHidden;
		}
		...
		{
			// IF YOU THINK ANIMAL TAMER PETS SOMEHOW RUIN YOUR GAME, YOU CAN INCREASE THIS VALUE
		...
		public static int CriticalToPets()
		{
			...
			if ( S_CriticalToPets < 1 ){ S_CriticalToPets = 0; } else if ( S_CriticalToPets > 100 ){ S_CriticalToPets = 100; }
		...
		{
			if ( S_SpellDamageIncreaseVsMonsters < 25 ){ S_SpellDamageIncreaseVsMonsters = 25; } else if ( S_SpellDamageIncreaseVsMonsters > 200 ){ S_SpellDamageIncreaseVsMonsters = 200; }
			return S_SpellDamageIncreaseVsMonsters;
		}

		public static int SpellDamageIncreaseVsPlayers()
		{
			if ( S_SpellDamageIncreaseVsPlayers < 25 ){ S_SpellDamageIncreaseVsPlayers = 25; } else if ( S_SpellDamageIncreaseVsPlayers > 200 ){ S_SpellDamageIncreaseVsPlayers = 200; }
		...
		public static int QuestRewardModifier()
		{
			...
			if ( S_QuestRewardModifier < 50 ){ S_QuestRewardModifier = 50; } else if ( S_QuestRewardModifier > 250 ){ S_QuestRewardModifier = 250; }
		...
		public static int PlayerLevelMod( int value, Mobile m )
		{
			...
			if ( S_PlayerLevelMod > 2 ){ S_PlayerLevelMod = 2.0; } else if ( S_PlayerLevelMod < 0.5 ){ S_PlayerLevelMod = 0.5; }

			double mod = 1.0;
				if ( m is PlayerMobile ){ mod = S_PlayerLevelMod; } // ONLY CHANGE THIS VALUE
		...
		{
			if ( S_HousesPerAccount < -1 ){ S_HousesPerAccount = -1; }
		...
		public static void AdditionalHitPoints( BaseCreature bc, int DungeonDifficulty )
		{
			...
			if ( DungeonDifficulty == 2 ){ HitPercents = Utility.RandomMinMax( 10, 20 )*0.01; }				// LEVEL 2 DUNGEONS
			else if ( DungeonDifficulty == 3 ){ HitPercents = Utility.RandomMinMax( 25, 35 )*0.01; }		// LEVEL 3 DUNGEONS
			else if ( DungeonDifficulty > 3 ){ HitPercents = Utility.RandomMinMax( 40, 55 )*0.01; }			// LEVEL 4 DUNGEONS
		...
		{
			int body = 214; if ( S_Leopards ){ body = 336; }
			return body;
		}
