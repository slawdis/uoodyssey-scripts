	...
	public abstract class Spell : ISpell
	{
		...
		public virtual int GetNewAosDamage( int bonus, int dice, int sides, bool playerVsPlayer, double scalar )
		{
			...
			int sdiBonus = AosAttributes.GetValue( m_Caster, AosAttribute.SpellDamage );
				if ( Server.Misc.MyServerSettings.SpellDamageIncreaseVsMonsters() > 0 && sdiBonus > Server.Misc.MyServerSettings.SpellDamageIncreaseVsMonsters() )
				{
		...
		public virtual bool CheckSequence()
		{
			...
			else if ( m_Scroll != null && !(m_Scroll is BaseMagicStaff) && !(m_Scroll is Runebook) && (m_Scroll.Amount <= 0 || m_Scroll.Deleted || m_Scroll.RootParent != m_Caster ) )
			{
				DoFizzle();
			}
