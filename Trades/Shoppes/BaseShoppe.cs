...
namespace Server.Items
{
	...
	public class BaseShoppe : Item, ISecurable
	{
		...
		{
			if ( shoppe is MorticianShoppe ){ from.CheckSkill( SkillName.Forensics, 0, 125 ); from.CheckSkill( SkillName.Alchemy, 0, 125 ); from.CheckSkill( SkillName.Necromancy, 0, 125 ); }
			else if ( shoppe is HerbalistShoppe ){ from.CheckSkill( SkillName.Alchemy, 0, 125 ); from.CheckSkill( SkillName.Cooking, 0, 125 ); from.CheckSkill( SkillName.AnimalLore, 0, 125 ); }
			...
			else if ( shoppe is BowyerShoppe ){ from.CheckSkill( SkillName.Fletching, 0, 125 ); }
        ...
        public ShoppeGump( BaseShoppe shoppe, Mobile from ): base( 25, 25 )
        {
			...
			else
			{
				...
				while ( entries > 0 )
				{
					...
					if ( customer != "" )
					{
						if ( taskDifficulty > 0 && shoppe.ShoppeTools >= taskTools && shoppe.ShoppeResources >= taskResources && ( shoppe.ShoppeGold + taskGold ) < 10001 )
						{
