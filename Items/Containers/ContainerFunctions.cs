    ...
    class ContainerFunctions
    {
		...
		public static void FillTheContainer( int level, LockableContainer box, Mobile opener )
		{
			...
			if ( Server.Misc.Worlds.IsOnSpaceship( opener.Location, opener.Map ) || IsSpaceCrate( box.ItemID ) )
			{
				...
				box.DropItem(it);
				BaseContainer.DropItemFix( it, opener, box.ItemID, box.GumpID );
			}
			...
			for ( int i = 0; i < ( filled ); ++i )
			{
				...
				if ( iRich > Utility.Random( 100 ) && IgnoreThisFiller == 0 )
				{
					...
					else if ( idropped is BaseIngot && Utility.RandomMinMax( 1, 8 ) == 1 && Worlds.GetMyWorld( opener.Map, opener.Location, opener.X, opener.Y ) == "the Underworld" && opener.Map == Map.SavagedEmpire ){ idropped = new XormiteIngot(); }
					else if ( idropped is BaseLeather && Utility.RandomMinMax( 1, 8 ) == 1 && Worlds.GetMyWorld( opener.Map, opener.Location, opener.X, opener.Y ) == "the Underworld" && opener.Map == Map.SavagedEmpire ){ idropped = new AlienLeather(); }
				...
				if ( iRich > Utility.Random( 1000 ) && IgnoreThisFiller == 0 && opener != null )
				{
					if ( Server.Misc.GetPlayerInfo.OrientalPlay( opener ) == true )
					{
						...
						box.DropItem( orient );
						BaseContainer.DropItemFix( orient, opener, box.ItemID, box.GumpID );
					}
				}
				if ( iRich > Utility.Random( 100 ) && IgnoreThisFiller == 0 )
				{
					...
					if (idropped is BlueBook){ idropped.Name = "Book"; idropped.Hue = Utility.RandomColor(0); idropped.ItemID = RandomThings.GetRandomBookItemID(); }
				...
				if ( iRich > Utility.Random( 1000 ) && IgnoreThisFiller != 2 )
				{
					...
					else if ( idropped is Runebook ){ idropped.ItemID = Utility.RandomList( 0x22C5, 0x0F3D, 0x4F50, 0x4F51, 0x5463, 0x5464 ); }
			...
			if ( iRich > Utility.Random( 5000 ) && IgnoreThisFiller != 2 ){ Item slay = DungeonLoot.RandomSlayer(); box.DropItem( slay ); BaseContainer.DropItemFix( slay, opener, box.ItemID, box.GumpID ); }
			...
			if ( iRich > Utility.Random( 7500 ) && opener != null && IgnoreThisFiller != 2 )
			{
				...
				if ( sandy == "the Serpent Island" )
				{
					...
					{
						case 0: Item s1 = new SandMiningBook();	box.DropItem( s1 ); BaseContainer.DropItemFix( s1, opener, box.ItemID, box.GumpID ); break;
						case 1: Item s2 = new GlassblowingBook();	box.DropItem( s2 ); BaseContainer.DropItemFix( s2, opener, box.ItemID, box.GumpID ); break;
						case 2: Item s3 = new Blowpipe();			box.DropItem( s3 ); BaseContainer.DropItemFix( s3, opener, box.ItemID, box.GumpID ); break;
					}
				}
			}
			...
			if ( iRich > Utility.Random( 7500 ) && IgnoreThisFiller != 2 )
			{
				...
				box.DropItem( golem );
				BaseContainer.DropItemFix( golem, opener, box.ItemID, box.GumpID );
			}
			else if ( iRich > Utility.Random( 7500 ) && IgnoreThisFiller == 2 )
			{
				Item robot = new RobotSchematics();
				box.DropItem( robot );
				BaseContainer.DropItemFix( robot, opener, box.ItemID, box.GumpID );
			}
			...
			if ( iRich > Utility.Random( 10000 ) && IgnoreThisFiller != 2 )
			{
				...
				box.DropItem( prison );
				BaseContainer.DropItemFix( prison, opener, box.ItemID, box.GumpID );
			}

			if ( iRich > Utility.Random( 5000 ) && IgnoreThisFiller != 2 )
			{
				Item quiver = Loot.RandomQuiver();
				LootMutate( opener, Server.LootPack.GetRegularLuckChance( opener ), quiver, box, level );
				box.DropItem( quiver );
				BaseContainer.DropItemFix( quiver, opener, box.ItemID, box.GumpID );
			}
			...
			if ( iRich > Utility.Random( 2500 ) )
			{
				...
				box.DropItem( lute );
				BaseContainer.DropItemFix( lute, opener, box.ItemID, box.GumpID );
			}
			...
			if ( iRich > Utility.Random( 1000 ) && IgnoreThisFiller != 2 )
			{
				...
				box.DropItem( rune );
				BaseContainer.DropItemFix( rune, opener, box.ItemID, box.GumpID );
			}
			...
			if ( iRich > Utility.Random( 200 ) && IgnoreThisFiller != 2 && Server.Items.BaseWizardStaff.HasStaff( opener ) )
			{
				...
				BaseContainer.DropItemFix( mageEye, opener, box.ItemID, box.GumpID );
			}
            ...
            if (  Utility.RandomMinMax( 0, 100 ) < ( level * 5 ) && CorpseNoFiller == 0 && IgnoreThisFiller != 2 )
			{
				if ( Server.Misc.GetPlayerInfo.EvilPlay( opener ) == true && Utility.RandomMinMax( 0, 10 ) == 10 )
				{
					...
					box.DropItem( evils );
					BaseContainer.DropItemFix( evils, opener, box.ItemID, box.GumpID );
				}
				else
				{
					...
					box.DropItem( relic );
					BaseContainer.DropItemFix( relic, opener, box.ItemID, box.GumpID );
				}
			}
			...
			for ( int i = 0; i < ( filled ); ++i )
			{
				...
				{
					Item item = Loot.RandomArmorOrShieldOrWeaponOrJewelry( Server.Misc.GetPlayerInfo.OrientalPlay( opener ) );
					LootMutate( opener, Server.LootPack.GetRegularLuckChance( opener ), item, box, level );
				}
			}
			if ( iRich > Utility.Random( 100 ) && ( IgnoreThisFiller == 0 || IgnoreThisFiller == 2 ) )
			{
				...
				if ( MyServerSettings.GetUnidentifiedChance() >= Utility.RandomMinMax( 1, 100 ) )
				{
					...
					box.DropItem( itemID );
					BaseContainer.DropItemFix( itemID, opener, box.ItemID, box.GumpID );
				...
				{
					box.DropItem( weapon );
					BaseContainer.DropItemFix( weapon, opener, box.ItemID, box.GumpID );
				}
			}
			if ( iRich > Utility.Random( 100 ) && ( IgnoreThisFiller == 0 || IgnoreThisFiller == 2 ) )
			{
				...
				{
					box.DropItem( armor );
					BaseContainer.DropItemFix( armor, opener, box.ItemID, box.GumpID );
				}
			}
			...
			for ( int i = 0; i < ( filled ); i++ )
			{
				if ( iRich > Utility.Random( 100 ) )
				{
					if ( Utility.RandomMinMax( 0, 4 ) == 1 )
					{
						...
						box.DropItem( item );
						BaseContainer.DropItemFix( item, opener, box.ItemID, box.GumpID );
					}
					else
					{
			...
			if ( iRich > Utility.Random( 100 ) && IgnoreThisFiller != 2 )
			{
				if ( MyServerSettings.GetUnidentifiedChance() >= Utility.RandomMinMax( 1, 100 ) )
				{
					...
					box.DropItem( myWand );
					BaseContainer.DropItemFix( myWand, opener, box.ItemID, box.GumpID );
				}
				else
				{
			...
			for ( int i = 0; i < fills; ++i )
			{
				if ( (level * 9) > Utility.Random( 100 ) && IgnoreThisFiller != 2 )
				{
					if ( MyServerSettings.GetUnidentifiedChance() >= Utility.RandomMinMax( 1, 100 ) )
					{
						...
						BaseContainer.DropItemFix( wizscroll, opener, box.ItemID, box.GumpID );
					}
					else
					{
						if ( Utility.Random( 2 ) == 1 ) { Item item = Loot.RandomScroll( 0, (rich+3), SpellbookType.Regular ); 	box.DropItem( item ); BaseContainer.DropItemFix( item, opener, box.ItemID, box.GumpID ); }
						else { Item item = Loot.RandomScroll( 0, ((rich*2)+3), SpellbookType.Necromancer ); 					box.DropItem( item ); BaseContainer.DropItemFix( item, opener, box.ItemID, box.GumpID ); }
					}
				}
			}
		}
			...
			for ( int i = 0; i < ( filled ); ++i )
			{
				if ( world == "the Town of Skara Brae" )
				{
					if ( iRich > Utility.Random( 100 ) )
					{
						...
						box.DropItem( note );
						BaseContainer.DropItemFix( note, from, box.ItemID, box.GumpID );
					}
				}
				else if ( Server.Misc.Worlds.IsOnSpaceship( box.Location, box.Map ) )
				{
					...
					{
						if ( Utility.RandomMinMax( 1, 10 ) == 1 ){ LootMutate( from, Server.LootPack.GetRegularLuckChance( from ), item, box, level ); }
					}
		...
		public static int BuildContainer( LockableContainer box, int thisHue, int thisItem, int thisGump, int thisDesign )
		{
			...
			Server.Items.BaseContainer.AlignContainerGumpSound( box );
		...
		public static void LootMutate( Mobile from, int luckChance, Item item, Container box, int level )
		{
			int boxID = 0;
			...
			if ( item != null )
			{
				...
				if ( item is BaseWeapon || item is BaseArmor || item is BaseJewel || item is BaseHat || item is BaseQuiver )
				{
					if ( item is BaseWeapon )
					{
						...
						if ( !IsSpaceCrate( boxID ) ){ item.Name = LootPackEntry.MagicItemName( item, from, Region.Find( from.Location, from.Map ) ); }
						Server.Misc.MorphingTime.MakeSpaceAceItem( item, from );
					}
				...
				{
					UnidentifiedItem itemID = new UnidentifiedItem();
					itemID.DropItem(item); BaseContainer.DropItemFix( itemID, from, box.ItemID, box.GumpID );
					...
					{
						box.DropItem( itemID ); BaseContainer.DropItemFix( itemID, from, box.ItemID, box.GumpID );
					}
		...
		public static void GiveRandomItem( Mobile from )
		{
			...
			else if ( found == 2 )
			{
				...
				else if ( loot is BaseIngot && Utility.RandomMinMax( 1, 8 ) == 1 && Worlds.GetMyWorld( from.Map, from.Location, from.X, from.Y ) == "the Underworld" && from.Map == Map.SavagedEmpire ){ loot = new XormiteIngot(); }
				else if ( loot is BaseLeather && Utility.RandomMinMax( 1, 8 ) == 1 && Worlds.GetMyWorld( from.Map, from.Location, from.X, from.Y ) == "the Underworld" && from.Map == Map.SavagedEmpire ){ loot = new AlienLeather(); }
			...
			else if ( found == 4 )
			{
				...
				if (loot is BlueBook){ loot.Name = "Book"; loot.Hue = Utility.RandomColor(0); loot.ItemID = RandomThings.GetRandomBookItemID(); }
				if ( (loot is MetalBox) )
				{
			...
			else if ( found == 6 )
			{
				...
				else if ( odd == 3 )
				{
					...
					Server.Misc.ContainerFunctions.LootMutate( from, Server.LootPack.GetRegularLuckChance( from ), loot, null, 1 );
				}
				else if ( odd == 4 )
				{
			...
			else if ( found == 7 )
			{
				...
				{
					loot = Loot.RandomArmorOrShieldOrWeaponOrJewelry( Server.Misc.GetPlayerInfo.OrientalPlay( from ) );
					Server.Misc.ContainerFunctions.LootMutate( from, Server.LootPack.GetRegularLuckChance( from ), loot, null, 1 );
				}
				else if ( odd == 3 )
				{
