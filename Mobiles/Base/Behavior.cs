		...
		public static void DressUpWizards( Mobile from )
		{
			...
			{ 
				Item robe = new Robe();
					...
					int wear = Utility.RandomMinMax( 0, 19 );
					if ( from.Body == 605 || from.Body == 606 ){ wear = Utility.RandomMinMax( 0, 18 ); } // ELVES CANNOT WEAR SORCERER ROBES
					...
					switch ( wear )
					{
						...
						case 19: robe = new SorcererRobe(); break;
					}
			...
			else
			{
				Item dress = new PlainDress();
					dress.Hue = Utility.RandomColor( clothHue );
					hoodColor = dress.Hue;
					from.AddItem( dress ); 
			}
			...
			{ 
				if ( Utility.Random( 2 ) == 1 )
				{ 
					int myHat = Utility.RandomMinMax( 0, 3 );
					...
					Item hat = new WizardsHat();
						...
						switch ( myHat )
						{
							...
							case 3: hat = new WizardHood(); break;
						}
						hat.Hue = hoodColor;
						hoodColor = hat.Hue;
						from.AddItem( hat ); 
			...
			{ 
				Cloak cloak = new Cloak( );
					cloak.Hue = Utility.RandomColor( clothHue );
					if ( hoodColor > 0 )
					{
						cloak.Hue = hoodColor;
					}
...
namespace Server.Mobiles
{
	...
	public abstract class BaseAI
	{
		...
		public virtual void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			{
				if ( ( m_Mobile is AerialServant || m_Mobile is HenchmanFamiliar || m_Mobile is FrankenPorter || m_Mobile is PackBeast || m_Mobile is GolemPorter ) && ( from == m_Mobile.ControlMaster ) )
				{
