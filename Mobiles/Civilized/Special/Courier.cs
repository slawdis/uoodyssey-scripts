...
namespace Server.Mobiles
{
	public class Courier : BasePerson
	{
		...
		public override void OnAfterSpawn()
		{
			...
			else
			{
				switch( Utility.RandomMinMax( 0, 5 ) )
				{
					...
					case 2: this.X=2264; this.Y=1692; this.Z=0; this.Map = Map.SerpentIsland; break;
		...
		public static void PickSearchLocation( CourierMail scroll, string DungeonNow, Mobile from, string alignment, string homeworld )
		{
			...
			if ( target is SearchBase && ( Worlds.GetDifficultyLevel( target.Location, target.Map ) <= GetPlayerInfo.GetPlayerDifficulty( from ) ) )
			{
			...
			for ( int i = 0; i < targets.Count; ++i )
			{
				...
				if ( xCount == aCount )
				{
					...
					scroll.MsgReward = Server.Misc.Worlds.GetDifficultyLevel( finding.Location, finding.Map ) + 2;
