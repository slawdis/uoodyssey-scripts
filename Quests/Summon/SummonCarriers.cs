...
namespace Server.Misc
{
    class SummonQuests
    {
		...
		public static int SummonCarriers( Mobile m, BaseCreature b, int Heat )
		{
			...
			else if ( reg.IsPartOf( "Dungeon Destard" ) && m is ShadowWyrm && Server.Misc.SummonQuests.IsInLocation( b.Home.X, b.Home.Y, m.Map, 5128, 847, Map.Lodor ) )
			{
				m.EmoteHue = 123;
				m.Name = "Dramulox";
			...
			else if ( reg.IsPartOf( "Terathan Keep" ) && m is BlackDragon && Server.Misc.SummonQuests.IsInLocation( b.Home.X, b.Home.Y, m.Map, 5307, 1611, Map.Lodor ) )
			{
				m.EmoteHue = 123;
			...
			else if ( reg.IsPartOf( "the Forgotten Halls" ) && m is LichKing && Server.Misc.SummonQuests.IsInLocation( b.Home.X, b.Home.Y, m.Map, 56, 3245, Map.SavagedEmpire ) ) // Shadow Lich already mutated 
			{
				m.EmoteHue = 123;
				m.Name = "Ulmarek";
