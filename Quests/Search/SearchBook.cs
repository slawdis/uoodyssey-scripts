...
namespace Server.Items
{
	public class SearchBook : Item
	{
		...
		public static string GetArtifactListForBook( int artifact, int part )
		{
			...
			if ( artifact == arty) { name="ElvenQuiver"; item="Elven Quiver"; } arty++;
			if ( artifact == arty) { name="EmbroideredOakLeafCloak"; item="Embroidered Oak Leaf Cloak"; } arty++;
			...
			if ( artifact == arty) { name="FesteringWound"; item="Fortified Arms"; } arty++;
			...
			if ( artifact == arty) { name="ShaMontorrossbow "; item="Shamino’s Best Crossbow"; } arty++;
